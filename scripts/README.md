...
`docs/restore-berg.sh`. Скрипт не создаёт, не обновляет и не вызывает объекты
других проектов.
...
11. записывает файл `<baseline>.restored` с результатами;
12. удаляет архив и checksum только после полного успеха.
