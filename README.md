...
TUI всегда использует `MDB_PATH`, выполняет локальную синхронизацию и публикацию
на хостинг. Перед запуском он показывает MDB и адреса обеих баз без учётных
данных, запрашивает подтверждение, выводит текущий этап и журнал `[PROFILE]`, а
после завершения показывает baseline, `delta_seq`, число изменений и время.
Параметр `--tui` не комбинируется с режимами `--incremental`, `--migrate`,
`--copy` и `--publish-hosting`.
