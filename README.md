...
Новая .NET-версия полной миграции Microsoft Access MDB в PostgreSQL и основа
для инкрементальной синхронизации. Проект никогда не изменяет MDB. PostgreSQL
изменяется только при явном указании `--copy` или `--migrate`.
